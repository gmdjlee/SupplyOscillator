    ...
    override fun refreshContent(e: Entry?, highlight: Highlight?) {
        try {
            ...
            if (index >= 0 && index < dates.size) {
                ...
                tvDate?.text = "N/A"
                Log.w(TAG, "MacdMarkerView: Index out of bounds: $index, size: ${dates.size}")
            }

            // MACD 값과 Signal 값 표시
            val text = try {
                if (index >= 0) {
                    val macd = if (index < macdValues.size) macdValues[index] else 0.0
                    val signal = if (index < signalValues.size) signalValues[index] else 0.0
                    String.format("MACD: %.2f\nSignal: %.2f", macd, signal)
                } else {
                    "N/A"
                }
            } catch (ex: Exception) {
